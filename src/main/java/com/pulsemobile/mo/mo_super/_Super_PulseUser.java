...
@com.percero.agents.sync.metadata.annotations.PropertyInterface(entityInterfaceClass=com.percero.agents.auth.vo.IUserIdentifier.class, propertyName="userIdentifier",params={@com.percero.agents.sync.metadata.annotations.PropertyInterfaceParam(name="paradigm", value="convergys")})
...
private List<TeamLeader> recentTeamLeaders;
public List<TeamLeader> getRecentTeamLeaders() {
	return this.recentTeamLeaders;
}

public void setRecentTeamLeaders(List<TeamLeader> value) {
	this.recentTeamLeaders = value;
}
...
objectJson += ",\"recentTeamLeaders\":[";
		
		if (getRecentTeamLeaders() != null) {
			int recentTeamLeadersCounter = 0;
			for(TeamLeader nextRecentTeamLeaders : getRecentTeamLeaders()) {
				if (recentTeamLeadersCounter > 0)
					objectJson += ",";
				try {
					objectJson += ((BaseDataObject) nextRecentTeamLeaders).toEmbeddedJson();
					recentTeamLeadersCounter++;
				} catch(Exception e) {
					// Do nothing.
				}
			}
...
objectJson += ",\"userSessions\":[";
	...
	protected void fromJson(JsonObject jsonObject) {
		...
		this.recentTeamLeaders = (List<TeamLeader>) JsonUtils.getJsonListPerceroObject(jsonObject, "recentTeamLeaders");
