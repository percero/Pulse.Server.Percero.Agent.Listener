...
@Component
public class ShiftStatusNotificationDAO extends
		SqlDataAccessProcObject<ShiftStatusNotification> implements
		IDataAccessObject<ShiftStatusNotification> {

	static final Logger log = Logger
			.getLogger(ShiftStatusNotificationDAO.class);

	public ShiftStatusNotificationDAO() {
		super();

		DAORegistry.getInstance().registerDataAccessObject(
				ShiftStatusNotification.class.getCanonicalName(), this);
	}
	...
	protected String getSelectShellOnlySQL() {
		return "SELECT \"SHIFT_STATUS_NOTIFICATION\".\"ID\" "
				+ selectFromStatementTableName + whereClause;
	}

	@Override
	protected String getSelectStarSQL() {
		return "SELECT \"SHIFT_STATUS_NOTIFICATION\".\"ID\"" + SQL_VIEW
				+ selectFromStatementTableName + whereClause;
	}
	...
	protected String getSelectByRelationshipStarSQL(String joinColumnName) {
		...
		return "SELECT \"SHIFT_STATUS_NOTIFICATION\".\"ID\"" + SQL_VIEW + " "
				...
				+ " WHERE \"SHIFT_STATUS_NOTIFICATION\"." + joinColumnName
				+ "=?";
	}
	...
	@Override
	protected ShiftStatusNotification extractObjectFromResultSet(ResultSet rs,
		...
		try {
			nextResult = (ShiftStatusNotification) MappedClass.forName(
		...
		if (!shellOnly) {
			...
			nextResult.setCreatedOn(DateUtils.utilDateFromSqlTimestamp(rs
					.getTimestamp("CREATED_ON")));

			nextResult.setType(rs.getString("TYPE"));

			nextResult.setShiftEndDate(DateUtils.utilDateFromSqlTimestamp(rs
					.getTimestamp("SHIFT_END_DATE")));
	...
	protected void setBaseStatmentInsertParams(
			ShiftStatusNotification perceroObject, PreparedStatement pstmt)
			throws SQLException {

		pstmt.setString(1, perceroObject.getID());
		JdbcHelper.setInt(pstmt, 2, perceroObject.getResolved() ? 1 : 0);
		pstmt.setDate(3,
				DateUtils.utilDateToSqlDate(perceroObject.getCreatedOn()));
		pstmt.setString(4, perceroObject.getType());
		pstmt.setDate(5,
				DateUtils.utilDateToSqlDate(perceroObject.getShiftEndDate()));
	...
	@Override
	protected void setPreparedStatmentInsertParams(
			ShiftStatusNotification perceroObject, PreparedStatement pstmt)
			throws SQLException {
	...
	@Override
	protected void setCallableStatmentInsertParams(
			ShiftStatusNotification perceroObject, CallableStatement pstmt)
			throws SQLException {
	...
	@Override
	protected void setPreparedStatmentUpdateParams(
			ShiftStatusNotification perceroObject, PreparedStatement pstmt)
			throws SQLException {
	...
	@Override
	protected void setCallableStatmentUpdateParams(
			ShiftStatusNotification perceroObject, CallableStatement pstmt)
			throws SQLException {
	...
	@Override
	public List<ShiftStatusNotification> findByExample(
			ShiftStatusNotification theQueryObject,
		...
		boolean useResolved = theQueryObject.getResolved() != null
		...
		boolean useCreatedOn = theQueryObject.getCreatedOn() != null
				&& (excludeProperties == null || !excludeProperties
						.contains("createdOn"));
		...
		boolean useType = StringUtils.hasText(theQueryObject.getType())
				&& (excludeProperties == null || !excludeProperties
						.contains("type"));
		...
		boolean useShiftEndDate = theQueryObject.getShiftEndDate() != null
				&& (excludeProperties == null || !excludeProperties
						.contains("shiftEndDate"));
		...
		boolean useTeamLeaderID = theQueryObject.getTeamLeader() != null
				&& (excludeProperties == null || !excludeProperties
						.contains("teamLeader"));
		...
		boolean useTimecardActivityID = theQueryObject.getTimecardActivity() != null
				&& (excludeProperties == null || !excludeProperties
						.contains("timecardActivity"));
