...
public class TimecardActivityDAO extends SqlDataAccessObject<TimecardActivity> implements IDataAccessObject<TimecardActivity> {
    ...
    public static final String SQL_VIEW = "SELECT  \"TIMECARD_ACTIVITY\".CENTRE || '-' ||\"TIMECARD_ACTIVITY\".POS as \"ID\", \"TIMECARD_ACTIVITY\".\"BILLABLE\" as \"NON_BILLABLE\", \"TIMECARD_ACTIVITY\".\"DESCRIPTION\" as \"DESCRIPTION\", \"TIMECARD_ACTIVITY\".\"POS\" as \"CODE\", \"TIMECARD_ACTIVITY\".CENTRE || '-' || \"TIMECARD_ACTIVITY\".POS as \"NAME\" FROM \"ESTART_ACTIVITY_CODE_VW\" \"TIMECARD_ACTIVITY\" ";
    ...
    private String whereClause = " ,(select ? As SQLID From Dual) WHERE \"TIMECARD_ACTIVITY\".CENTRE=SUBSTR(SQLID,0,INSTR(SQLID,'-')-1) and \"TIMECARD_ACTIVITY\".POS=SUBSTR(SQLID,INSTR(SQLID,'-')+1)";
    ...
    protected TimecardActivity extractObjectFromResultSet(ResultSet rs, Boolean shellOnly) throws SQLException {
        ...
        if (!shellOnly) {
            nextResult.setNonBillable(rs.getString("NON_BILLABLE"));
