...
//	public static final String CONNECTION_FACTORY_NAME = "jdbc:mysql://pulse.cta6j6w4rrxw.us-west-2.rds.amazonaws.com:3306/Pulse?autoReconnect=true";
	...
	protected String getSelectShellOnlySQL() {
		return "SELECT workdurationnotification.ID FROM WorkDurationNotification workdurationnotification WHERE workdurationnotification.ID=?";
	}
	
	@Override
	protected String getSelectStarSQL() {
		return "SELECT workdurationnotification.ID,workdurationnotification.externalID,workdurationnotification.message,workdurationnotification.name,workdurationnotification.date,workdurationnotification.type,workdurationnotification.agent_ID,workdurationnotification.lOBConfiguration_ID,workdurationnotification.teamLeader_ID,workdurationnotification.lOBConfigurationEntry_ID FROM WorkDurationNotification workdurationnotification WHERE workdurationnotification.ID=?";
	}
	
	@Override
	protected String getSelectAllShellOnlySQL() {
		return "SELECT workdurationnotification.ID FROM WorkDurationNotification workdurationnotification ORDER BY ID";
	}
	
	@Override
	protected String getSelectAllShellOnlyWithLimitAndOffsetSQL() {
		return "SELECT workdurationnotification.ID FROM WorkDurationNotification workdurationnotification ORDER BY workdurationnotification.ID LIMIT ? OFFSET ?";
	}
	
	@Override
	protected String getSelectAllStarSQL() {
		return "SELECT workdurationnotification.ID,workdurationnotification.externalID,workdurationnotification.message,workdurationnotification.name,workdurationnotification.date,workdurationnotification.type,workdurationnotification.agent_ID,workdurationnotification.lOBConfiguration_ID,workdurationnotification.teamLeader_ID,workdurationnotification.lOBConfigurationEntry_ID FROM WorkDurationNotification workdurationnotification ORDER BY workdurationnotification.ID";
	}
	
	@Override
	protected String getSelectAllStarWithLimitAndOffsetSQL() {
		return "SELECT workdurationnotification.ID,workdurationnotification.externalID,workdurationnotification.message,workdurationnotification.name,workdurationnotification.date,workdurationnotification.type,workdurationnotification.agent_ID,workdurationnotification.lOBConfiguration_ID,workdurationnotification.teamLeader_ID,workdurationnotification.lOBConfigurationEntry_ID FROM WorkDurationNotification workdurationnotification ORDER BY workdurationnotification.ID LIMIT ? OFFSET ?";
	}
	
	@Override
	protected String getCountAllSQL() {
		return "SELECT COUNT(ID) FROM WorkDurationNotification workdurationnotification";
	}
	
	@Override
	protected String getSelectInStarSQL() {
		return "SELECT workdurationnotification.ID,workdurationnotification.externalID,workdurationnotification.message,workdurationnotification.name,workdurationnotification.date,workdurationnotification.type,workdurationnotification.agent_ID,workdurationnotification.lOBConfiguration_ID,workdurationnotification.teamLeader_ID,workdurationnotification.lOBConfigurationEntry_ID FROM WorkDurationNotification workdurationnotification WHERE workdurationnotification.ID IN (?)";
	}
	
	@Override
	protected String getSelectInShellOnlySQL() {
		return "SELECT workdurationnotification.ID FROM WorkDurationNotification workdurationnotification WHERE workdurationnotification.ID IN (?)";
	}

	@Override
	protected String getSelectByRelationshipStarSQL(String joinColumnName) {
		return "SELECT workdurationnotification.ID,workdurationnotification.externalID,workdurationnotification.message,workdurationnotification.name,workdurationnotification.date,workdurationnotification.type,workdurationnotification.agent_ID,workdurationnotification.lOBConfiguration_ID,workdurationnotification.teamLeader_ID,workdurationnotification.lOBConfigurationEntry_ID FROM WorkDurationNotification workdurationnotification WHERE workdurationnotification." + joinColumnName + "=?";
	}
	
	@Override
	protected String getSelectByRelationshipShellOnlySQL(String joinColumnName) {
		return "SELECT workdurationnotification.ID FROM WorkDurationNotification workdurationnotification WHERE workdurationnotification." + joinColumnName + "=?";
	}

	@Override
	protected String getFindByExampleSelectShellOnlySQL() {
		return "SELECT workdurationnotification.ID FROM WorkDurationNotification workdurationnotification ";
	}

	@Override
	protected String getFindByExampleSelectAllStarSQL() {
		return "SELECT workdurationnotification.ID,workdurationnotification.externalID,workdurationnotification.message,workdurationnotification.name,workdurationnotification.date,workdurationnotification.type,workdurationnotification.agent_ID,workdurationnotification.lOBConfiguration_ID,workdurationnotification.teamLeader_ID,workdurationnotification.lOBConfigurationEntry_ID FROM WorkDurationNotification workdurationnotification ";
	}
	
	@Override
	protected String getInsertIntoSQL() {
		return "INSERT INTO WorkDurationNotification (ID,externalID,message,name,date,type,agent_ID,lOBConfiguration_ID,teamLeader_ID,lOBConfigurationEntry_ID) VALUES (?,?,?,?,?,?,?,?,?,?)";
	}
	
	@Override
	protected String getUpdateSet() {
		return "UPDATE WorkDurationNotification SET externalID=?,message=?,name=?,date=?,type=?,agent_ID=?,lOBConfiguration_ID=?,teamLeader_ID=?,lOBConfigurationEntry_ID=? WHERE ID=?";
	}
	
	@Override
	protected String getDeleteFromSQL() {
		return "DELETE FROM WorkDurationNotification WHERE ID=?";
	}
